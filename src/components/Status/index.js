...
import { Avatar, Dropdown, Menu, Icon, Badge, Select } from "antd";
...
import {
  ...
  useMyAddress,
  readMyAddress,
} from "../HOC/DApp/SubstrateAccountProvider";
      ...
      <Menu className="status-dropdown">
        {user ? (
          <>
            {!account ? (
              ...
              <>
                <a className="connect-wallet">
                  <BalanceAnnotation
                    accountSelected={accountAddressSelected}
                    style={{ marginRight: 5 }}
                  />
                  <Select
                    style={{ width: 220 }}
                  ...
                  >
                    {accountOption != null
                      ? accountOption.map((account, index) => (
                          <Option
                            onClick={() => onWalletChange(account)}
                            key={index}
                            value={account.key}
                          >
                            <strong>{account.text.substr(0, 8)} </strong>-{" "}
                            {account.key.substr(0, 8)}....
                            {account.key.substr(account.key.length - 4)}
                          </Option>
                        ))
                      : null}
                  </Select>
            ...
            <Link to="/notifications" onClick={clearNotificationCount}>
  ...
  return collapsed ? (
    <div className="navigation collapsed" ref={containerRef}>
      ...
      <div className="nav-btn">
        <a>
          <Icon type={drawerOpen ? "up" : "down"} onClick={toggleDrawer} />
        </a>
      </div>
    </div>
  ) : (
