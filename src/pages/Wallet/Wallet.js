...
import {
  ...
  useMyAddress,
  readMyAddress,
} from "../../components/HOC/DApp/SubstrateAccountProvider";
...
function Wallet({ /*address,*/ user, intl }) {
  ...
  const [{ isUsable, signer }, setSigner] = useState({
    isUsable: true,
    signer: null,
  });
    ...
    (async function () {
      ...
      if (!response?.isSuccess) {
        Notification.displayErrorMessage(
          intl.formatMessage({ id: "wallet.token.fetch.error" })
        );
      }
  ...
  const initApi = async (api, keyring) => {
    if (api && keyring) {
      ...
      if (savedAddress) {
        ...
        const selectedCurrentAddress = _.filter(keyringOptions, (o) => {
          return o.value == savedAddress;
        });
                ...
                <div className="wallet-address">
                  <span className="wallet-name">
                    {account && account.text
                      ? account.text && account.text.length > 15
                        ? `${account.text.substr(0, 10)} ...`
                        : account.text
                      : null}
                  </span>
                  <span className="wallet-address-detail">
                    {account && account.key && account.key.substr(0, 8)}....
                    {account &&
                      account.key &&
                      account.key.substr(account.key.length - 8)}
                  </span>
                </div>
                <div className="wallet-setting">
                  <div className="edit-section">
                    <a onClick={() => setIsChooseAccountModal(true)}>Edit</a>
                  </div>
                  <div className="copy-section">
                    <a onClick={() => copyAddress(account.key)}>Copy</a>
                  </div>
                </div>
              </Card>
            </Col>
        ...
        >
          <List
            ...
            renderItem={(item) => (
              <List.Item
                key={item.key}
                onClick={() => onAccountListChange(item)}
              >
                <h4>{item.text}</h4>
                <div>
                  <Icon
                    className={`check-mark ${
                      accountAddressSelected == item.key && "active"
                    }`}
                    type="check"
                  />
                </div>
...
function MainBalance(props) {
  ...
  return accountSelected ? <h3>{accountBalance}</h3> : null;
}
